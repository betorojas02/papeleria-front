        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            >
              ...
              <td class="py-3 px-4 text-right font-bold text-slate-800 text-sm">{{ formatCurrency(sale.total) }}</td>
      ...
      >
        <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
          ...
          <div class="flex justify-between items-center p-4 border-b border-slate-200">
            <h3 class="text-xl font-bold text-slate-800">Detalles de Venta #{{ selectedSale.id.toString().slice(-6) }}</h3>
            <button @click="closeDetails" class="p-2 rounded-full hover:bg-slate-100 transition-colors">
          ...
          <div class="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p class="text-xs text-slate-500 uppercase font-semibold mb-1">Fecha y Hora</p>
                <p class="text-slate-800 text-sm">{{ formatDate(selectedSale.createdAt).full }}</p>
              </div>
              <div>
                <p class="text-xs text-slate-500 uppercase font-semibold mb-1">Cliente</p>
                <div v-if="selectedSale.customer" class="flex items-center gap-2 text-slate-800 text-sm">
                  <UserIcon class="w-4 h-4 text-slate-400" />
                  <span>{{ selectedSale.customer.firstName }} {{ selectedSale.customer.lastName }}</span>
                </div>
                <p v-else class="text-slate-500 italic text-sm">Público General</p>
              </div>
            </div>
          ...
          <div class="bg-primary-50 rounded-xl p-4 border border-primary-100">
            ...
            <div class="space-y-2 text-sm">
               <div class="flex justify-between items-center text-slate-600" v-for="pay in selectedSale.payments" :key="pay.id">
                  ...
                  <span class="font-medium text-slate-800">{{ formatCurrency(pay.amount || 0) }}</span>
               </div>
